...
export const TechnologySection: React.FC = () => {
  ...
  return (
    <section id="technology" className="py-24 bg-transparent relative overflow-hidden border-t border-forest-900/10">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        
        {/* Header Tag */}
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-12">
          <div>
            <div className="flex items-center gap-2 text-xs font-mono text-emerald uppercase tracking-widest mb-3 font-bold">
              <Cpu className="w-4 h-4" />
              <span>TECHNOLOGY STACK</span>
            </div>
            <h2 className="text-3xl sm:text-4xl lg:text-5xl font-extrabold text-charcoal tracking-tight font-sans">
              "Technologies We{' '}
          ...
          <div className="mt-4 md:mt-0 p-4 rounded-2xl bg-emerald-soft border border-forest-900/10 font-mono text-xs text-forest-900 font-semibold max-w-md">
        ...
        {/* Infinite Horizontal Ticker Showcase */}
        <div className="relative overflow-hidden py-4 border-y border-forest-900/10 bg-ivory-50 rounded-2xl">
          <div className="animate-infinite-scroll flex items-center gap-4">
            {tickerItems.map((tech, idx) => (
              <div
                key={`${tech.name}-${idx}`}
                className="px-5 py-3 rounded-xl bg-ivory-100 border border-forest-900/10 flex items-center gap-3 shrink-0 hover:border-emerald/40 transition-colors shadow-forest-subtle"
              >
